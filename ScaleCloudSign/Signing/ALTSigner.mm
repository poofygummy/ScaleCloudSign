...
- (NSProgress *)signAppAtURL:(NSURL *)appURL provisioningProfiles:(NSArray<ALTProvisioningProfile *> *)profiles completionHandler:(void (^)(BOOL success, NSError *error))completionHandler
{
    ...
    if ([appURL.pathExtension.lowercaseString isEqualToString:@"ipa"])
    {
        ...
        if (appBundleURL == nil)
        {
            finish(NO, [NSError errorWithDomain:ScaleCloudSignErrorDomain code:ALTErrorMissingAppBundle userInfo:@{NSUnderlyingErrorKey: error}]);
            return progress;
        }
    }
    else
    ...
    if (appBundle == nil)
    {
        finish(NO, [NSError errorWithDomain:ScaleCloudSignErrorDomain code:ALTErrorInvalidApp userInfo:nil]);
        return progress;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSError * (^prepareApp)(ALTApplication *) = ^NSError *(ALTApplication *app) {
            ...
            if (profile == nil)
            {
                return [NSError errorWithDomain:ScaleCloudSignErrorDomain code:ALTErrorMissingProvisioningProfile userInfo:nil];
            }
        ...
        {
            NSError *error = [NSError errorWithDomain:ScaleCloudSignErrorDomain code:ALTErrorUnknown userInfo:@{
